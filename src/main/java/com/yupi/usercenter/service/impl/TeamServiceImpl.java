...
//        2. 依据请求参数构造查询条件
        {
            ...
            if (teamStatusByValue != null){
                wrapper.eq("status", teamStatusByValue.getStatus());
            }
        }
...
//        3. 关联创建队伍的用户
        ...
        for (Team team : teamList) {
            ...
            QueryWrapper<UserTeam> wrapper2 = new QueryWrapper<UserTeam>().eq("teamId", team.getId());
            long amount = userTeamService.count(wrapper2);
            teamUserVO.setHasJoinNum(amount);
            wrapper2.eq("userId", loginUser.getId());
            long hasJoinedTeamCurrentUser = userTeamService.count(wrapper2);
            // 有可能用户由于系统原因重复加入队伍，导致加入量为2,3,4....
            teamUserVO.setHasJoin(hasJoinedTeamCurrentUser >= 1);
...
//          2. 新的状态如果和旧状态一致则不修改。如果从加密状态变为公开或私密，需要清除密码。如果从公开或私密变为加密，必须传入密码。密码需要加盐。
    ...
    public boolean joinTeam(TeamJoinRequest teamJoinRequest, User userLogin) {
        synchronized (this){
            ...
            Long expectedJoinTeamId = teamJoinRequest.getTeamId();
